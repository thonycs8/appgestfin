...
-- Criar índices básicos verificando se as colunas existem
DO $$
BEGIN
  -- Índice único para clerk_id (verificar se a coluna existe)
  ...
  -- Verificar se a coluna is_active existe antes de criar o índice
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'is_active') THEN
    CREATE INDEX IF NOT EXISTS idx_users_active ON users(is_active);
  END IF;
  
  -- Verificar se a coluna last_sign_in_at existe antes de criar o índice
  ...
  -- Verificar se a coluna role existe antes de criar o índice
