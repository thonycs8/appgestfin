/*
  # User Management and Authentication Migration
  
  This migration adds Clerk integration support to the users table and creates
  necessary functions and policies for authentication and authorization.
  
  1. Adds Clerk-specific columns to users table
  2. Creates authentication helper functions
...
DO $$
BEGIN
  -- Pequena pausa para garantir que as alterações foram aplicadas
  PERFORM pg_sleep(0.1);
END $$;

-- Criar índices básicos para email e id (sempre presentes)
DO $$
BEGIN
  IF NOT EXISTS (
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE tablename = 'users' AND indexname = 'idx_users_id'
  ) THEN
    CREATE INDEX idx_users_id ON users(id);
  END IF;
END $$;

-- Criar índice para clerk_id se a coluna existir
DO $$
DECLARE
  col_exists boolean;
  idx_exists boolean;
BEGIN
  ...
  SELECT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE tablename = 'users' AND indexname = 'idx_users_clerk_id_unique'
  ) INTO idx_exists;
  
  IF col_exists AND NOT idx_exists THEN
    CREATE UNIQUE INDEX idx_users_clerk_id_unique ON users(clerk_id) WHERE clerk_id IS NOT NULL;
  END IF;
END $$;

-- Criar índice para is_active se a coluna existir
DO $$
DECLARE
  col_exists boolean;
  idx_exists boolean;
