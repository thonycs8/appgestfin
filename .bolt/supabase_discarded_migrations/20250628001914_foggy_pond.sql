/*
  # Fix user_id column types and recreate views

  1. Database Changes
    - Drop existing views and policies that depend on user_id columns
    - Convert user_id columns from uuid to text across all tables
    - Create users table with text primary key
    - Recreate all RLS policies with text user_id references
    - Recreate views without unsupported parameters
    - Add foreign key constraints

  2. Security
    - Enable RLS on users table
    - Recreate all RLS policies for proper user isolation
    - Add policies for Stripe-related tables

  3. Functions and Triggers
    - Create function to automatically create user records from Clerk
...
-- Step 9: Recreate views with updated column types (without security_definer parameter)
