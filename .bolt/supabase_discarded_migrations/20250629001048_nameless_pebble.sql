/*
  # Melhorar integração de autenticação Clerk/Supabase

  1. Atualizar tabela users
    - Adicionar campos necessários para integração com Clerk
    - Melhorar estrutura de dados do usuário
    - Adicionar campos de auditoria

  2. Criar função para sincronização automática de usuários
    - Função para criar/atualizar usuário automaticamente
    - Trigger para garantir consistência dos dados

  3. Melhorar RLS (Row Level Security)
    - Políticas mais robustas
    - Melhor integração com JWT do Clerk
...
-- Criar função para extrair user_id do JWT
...
BEGIN
  ...
  INSERT INTO public.users (
    ...
    COALESCE(user_data ->> 'sub', user_data ->> 'user_id'),
...
-- Melhorar função auto_set_user_id para usar a nova função auth.user_id()
...
-- Atualizar políticas RLS para usar a nova função auth.user_id()
...
DECLARE
  ...
  stats jsonb;
BEGIN
  target_user_id := COALESCE(user_id_param, auth.user_id());
