/*
  ...
  1. New Tables
    - Enhanced `users` table with Clerk integration fields
    - Added columns: clerk_id, username, avatar_url, phone, email_verified, phone_verified, is_active, last_sign_in_at, metadata, role

  2. Security
    - Enable RLS on all tables
    - Add comprehensive policies for user data access
    - Create secure functions for JWT handling

  3. Functions
    - get_current_user_id(): Extract user ID from JWT
    - get_jwt_claims(): Get JWT claims safely
    - auto_set_user_id(): Automatically set user_id on inserts
    - ensure_user_before_operation(): Ensure user exists before operations
    - sync_user_from_jwt(): Sync user data from JWT
    - get_user_financial_stats(): Get user statistics
    - cleanup_old_data(): Cleanup old data (placeholder)
...
-- Criar índices básicos verificando se as colunas existem APÓS criá-las
DO $$
DECLARE
  col_exists boolean;
BEGIN
  ...
  -- Índices básicos sempre presentes
