/*
  # Fix user_id column types for Clerk compatibility
  ...
  1. Database Schema Updates
    ...
    - Recreate RLS policies with text-based user IDs
    - Add foreign key constraints and automation
  ...
  3. Automation
    - Trigger to create user records when Clerk users sign up
    - Updated_at triggers for timestamp management
*/

-- Step 1: Drop ALL RLS policies that might depend on user_id columns
-- Get all policies that reference user_id and drop them
DO $$
DECLARE
...
-- Step 4: Update user_id columns from uuid to text for all tables
-- Categories table
DO $$
BEGIN
  IF EXISTS (
...
-- Stripe subscriptions policies (recreate the missing ones)
...
-- Stripe orders policies (recreate if needed)
