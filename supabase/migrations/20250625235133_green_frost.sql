/*
  ...
  2. Security
    - Recreate RLS policies with text user_id references
    - Recreate views without security_definer parameter
    - Add triggers for automatic user creation
*/
...
-- Step 9: Recreate views without security_definer parameter
...
-- Step 15: Create helper function to ensure user exists
CREATE OR REPLACE FUNCTION ensure_user_exists(user_id text, user_email text DEFAULT NULL)
RETURNS void AS $$
BEGIN
  INSERT INTO users (id, email, created_at)
  VALUES (user_id, COALESCE(user_email, user_id || '@unknown.com'), now())
